...
\subsubsection{\LaTeX 関係で作って公開しているもの}
\begin{itemize}
  \item multipagebox.sty

    ページをまたげる囲み罫のパッケージです。

    \url{http://hisashim.livejournal.com/398344.html}

  \item vfdata-otf-ptex

    OpenTypeを扱うためのOTFパッケージのDebianパッケージです。

    \url{http://sourceforge.net/projects/ideotype}
\end{itemize}

\subsubsection{ \LaTeX について知りたいこと}
\begin{itemize}
  \item 次世代日本語 \TeX の動向（特にUnicodeサポート）
    \begin{itemize}
    \item ep\TeX, up\TeX, eup\TeX
    \item ptexlive
    \end{itemize}
...
\subsubsection{ \LaTeX と関係ないけれど知りたいこと}
\begin{itemize}
  \item tex4htを \pTeX で使う方法
\end{itemize}
...
\dancersection{事前課題の反省会}{上川 純一}

\subsection{はじまりはいつも雨}

今日は雨だった。
しとしとと降る雨の中僕はとぼとぼと歩いた。
まだあんさんぶるは遠いのか?
あんさんぶるでは宗教については論じてはいけないらしいので宗教論争はしない
ことにする。\texttt{vi} vs \texttt{emacs} とかはもってのほか。
\footnote{これは注釈です。}

みんな\textbf{emacs}教のふりをして、すなおにしたがっている。
いつ\texttt{vi}教徒が反乱をおこしてもおかしくない。
殺伐としている。
...
\subsection{そして電設へ}
\subsubsection{そして電設へ}
\subsubsection{そして電設へ}
\subsubsection{そして電設へ}
\subsubsection{そして電設へ}

\santaku{きょういちばんはずかしいかっこうをしているのはだれ?}
{小林さん}
{山田さん}
{岩松さん}
{A}

この資料をみるためには\url{http://git.debian.org//////......}を参照して
ください。

ながい文章をかいているまんなかでURLを入力してもちゃんと改行されるのか?
\url{http://www.netfort.gr.jp/~dancer/}
...
\begin{commandline}
 ...
 /dev/sda3             9.2G  4.7G  4.1G  54% /
 tmpfs                 486M     0  486M   0% /lib/init/rw
 udev                   10M  104K  9.9M   2% /dev
 tmpfs                 486M     0  486M   0% /dev/shm
 tmpfs                 486M  104K  486M   1% /tmp
 /dev/mapper/vgcore2duo-home
                       40G   26G   13G  68% /home
\end{commandline}

\newcommand{\iwamatsunobuhiro}[1]{岩松 信洋(#1)}
 
{\HUGE \iwamatsunobuhiro{29}}
...
\includegraphics[width=0.4\hsize]{image200811/toilet.png}

\includegraphics[width=0.8\hsize]{../base-image/openlogo-nd.eps}
...
\hrule
\vspace{2mm}
\includegraphics[width=2cm]{../base-image/openlogo-nd.eps}
